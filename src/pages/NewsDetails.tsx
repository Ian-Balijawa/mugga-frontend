		...
		<div className="min-h-screen bg-gradient-to-b from-white to-gray-50">
			<div className="container py-12">
				...
				>
					...
					{/* Related Articles */}
					{isLoadingRelated ? (
						<div className="mt-16 text-center">Loading related articles...</div>
					) : (
						<div className="mt-16">
							<h2 className="text-2xl font-bold mb-6">Related Articles</h2>
							<div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
								{relatedArticles.map((relatedArticle) => (
									<Link to={`/news/${relatedArticle.id}`} key={relatedArticle.id}>
										<motion.div
											initial={{ opacity: 0, y: 20 }}
											animate={{ opacity: 1, y: 0 }}
											transition={{ duration: 0.8 }}
										>
											<div className="card h-full overflow-hidden hover:shadow-lg transition-all">
												<div className="aspect-video w-full overflow-hidden">
													{relatedArticle.videoUrl ? (
														<VideoPlayer
															videoUrl={relatedArticle.videoUrl}
															thumbnail={relatedArticle.imageUrl}
